...
WHERE AverageProductPrice > (
    SELECT AVG(AverageProductPrice)
    FROM SupplierAveragePrice
);
...
WITH DifferentCategories AS (
    SELECT
        COUNT(DISTINCT ca.CategoryID) AS DistinctCategories,
        cu.CustomerID,
...
WHERE DistinctCategories = (
    SELECT MAX(DistinctCategories)
    FROM DifferentCategories
);
...
SELECT
    ...
    TotalUnitsInStock
FROM Stock
WHERE TotalUnitsInStock < (
    SELECT AVG(TotalUnitsInStock)
    FROM Stock
);